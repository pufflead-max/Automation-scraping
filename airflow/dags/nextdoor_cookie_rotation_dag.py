...
def rotate_nextdoor_owner_cookies(**context):
    ...
    with sync_playwright() as p:
        ...
        # Headless Toggle
        is_headless = Variable.get("nextdoor_headless", default_var="true").lower() == "true"

        display = None
        if not is_headless:
            from pyvirtualdisplay import Display
            print(" Starting Virtual Display (Xvfb) for non-headless mode...")
        ...
        try:
            ...
            try:
                ...
                print(f" Root domain navigation warning: {root_err}. Continuing to news_feed...")
            ...
            if "login" in current_url or "signup" in current_url or "identify" in page.content().lower():
                ...
                print(f" Filling email: {printable_email}")
            ...
            print(" Final verification. Waitxing for news_feed redirection...")
            try:
                page.wait_for_url(lambda url: "login" not in url.lower(), timeout=30000)
                print(f" Redirection complete. Target URL: {page.url}")
            except Exception as wait_err:
                print(f" Redirection wait exceeded. Taking debug screenshot...")
                debug_path = f"/opt/airflow/logs/nd_login_debug_{datetime.now().strftime('%H%M%S')}.png"
                try: page.screenshot(path=debug_path)
                except: pass
                print(f" Screenshot saved to {debug_path} for investigation.")
            ...
            if not has_session:
                print(" CRITICAL: 'ndbr_at' session cookie NOT found. Login likely failed or was blocked.")
            else:
                print(" SUCCESS: 'ndbr_at' session cookie retrieved.")

            manager.save_cookies(owner_email, 'nextdoor', cookies)

            # Sync to local JSON file
            try:
                # Use a directory Airflow definitely has access to
                cookie_file_path = "/tmp/nextdoor_cookies.json"
                print(f" Syncing cookies to temporary path: {cookie_file_path}")
                with open(cookie_file_path, 'w') as f:
                    json.dump(cookies, f, indent=2)
                print(f" Local sync successful.")
            except Exception as fe:
                print(f" Error syncing: {fe}")
