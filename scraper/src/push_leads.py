...
def push_leads(source: str, limit: int = None, force: bool = False, user_email: str = None):
    ...
    if not leads:
        print(f"ℹ No new leads for {source} in {col}.")
        return
    ...
    if not buyer_leads:
        print(f" No fresh buyer leads (last 48h) for {source} after filtering.")
        return
    ...
    for user_email, user_leads in leads_by_user.items():
        ...
        for i, lead in enumerate(user_leads):
            ...
            if record_id:
                ...
                print(f" [{total_processed}/{len(buyer_leads)}] Failed: {lead.get('title', 'Unknown')[:50]}")
